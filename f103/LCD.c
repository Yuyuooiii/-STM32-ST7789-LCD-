 ...
 * - BL: 背光控制 (默认接高电平)
 *
 * * 使用SPI1进行数据传输，配置为主模式, 2分频F103为36MHZ。
 * * 使用DMA1的通道3进行数据传输。
 */
...
#define LCD_DC_LOW()   (GPIOB->BRR = GPIO_Pin_0)
#define LCD_CS_HIGH()  (GPIOB->BSRR = GPIO_Pin_1)
#define LCD_CS_LOW()   (GPIOB->BRR = GPIO_Pin_1)
#define LCD_RST_HIGH() (GPIOB->BSRR = GPIO_Pin_10)
#define LCD_RST_LOW()  (GPIOB->BRR = GPIO_Pin_10)
...
void SPI_Config(void)
{
    ...
    SPI_InitStruct.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2; // 36MHz @72MHz/2
...
void DMA_Config(void)
{
    ...
    DMA_InitStructure.DMA_MemoryBaseAddr     = 0; // 后面用到再具体修改
    DMA_InitStructure.DMA_DIR                = DMA_DIR_PeripheralDST;
    DMA_InitStructure.DMA_BufferSize         = 233; // 后面用到再具体修改
...
void LCD_SPI_Transmit_DMA(const uint8_t *data, uint32_t length)
{
    ...
    DMA_SetCurrDataCounter(DMA1_Channel3, length); // 设置DMA传输长度
    DMA1_Channel3->CMAR = (uint32_t)data;          // 设置内存地址
